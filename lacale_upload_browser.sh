...
printf '%s\n' 'import os as _os' >> "$PY"
printf '%s\n' 'file_basename = _os.path.splitext(_os.path.basename(file_path))[0]' >> "$PY"
printf '%s\n' 'info = {"name": file_basename, "piece length": piece_length, "pieces": bytes(pieces), "length": file_size, "private": 1, "source": "La Cale"}' >> "$PY"
...
while IFS= read -r MOVIE_JSON; do
    ...
    if [ -f "${WORK_DIR}/${RELEASE_NAME}.nfo" ] && [ -s "${WORK_DIR}/${RELEASE_NAME}.nfo" ]; then
        ...
        sed "s|Complete name[[:space:]]*:[[:space:]]*/[^ ]*|Complete name                            : $FILENAME|g" \
    ...
    NAS_SAVE_DIR=$(dirname "$NAS_FILE_PATH")
    ...
    if echo "$QB_LOGIN" | grep -qi 'Ok'; then
        QB_ADD=$(curl -sf --max-time 30 \
            ...
            -F "paused=false" \
            2>/dev/null)
        echo "$QB_ADD" | grep -qi 'Ok' && QB_OK=1
    fi

    if [ "$QB_OK" -eq 1 ]; then
        log "  ✓ Torrent en seed dans qBittorrent"
    else
